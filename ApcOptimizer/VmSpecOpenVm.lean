...
/-! Draft `HostChip`s for OpenVM, built against `ApcOptimizer.OpenVM`'s bus semantics
    ...
    unconstrained words starting at the pointer). Assembled into a concrete `openVm : Vm p` at
    the bottom.

    `Vm.inputChunk`/`Vm.outputArray : BusState p → List (ZMod p)` need to read an *ordered
...
/-- Build a `BusState p` from a list of already-evaluated bus interactions: pointwise, the net
    ...
    the same rule `Circuit.allEffects`/`VmAssignment.netContribution` use. -/
...
/-- The memory-initialization host chip (default bus `1`): sends (multiplicity `1`, OpenVM's
    `setNew` polarity) each touched address's initial word at timestamp `0`, and nothing else.
    What "initial" means for a given address — typically all-zero, except addresses the input
    chip supplies a value for instead — is left to `initialWord`. -/
def memoryInitHostChip (initialWord : ZMod p → ZMod p → Vector (ZMod p) 4)
    (memBusId : Nat := 1) : HostChip p where
  legal contribution :=
    ∀ message : BusMessage p, contribution message ≠ 0 →
      ...
      ∃ f : MemoryPayload p, memoryPayload? message.2 = some f ∧
        f.data = initialWord f.addressSpace f.pointer ∧ message.2.getLast? = some 0
  singleton := True

/-- The memory-finalization host chip (default bus `1`): the last receive (multiplicity `-1`,
    OpenVM's `getPrevious` polarity) of each touched address. Legal to receive any final word,
    since (unlike initialization) what a real execution actually leaves behind isn't knowable in
    advance. -/
...
def outputHostChip (memBusId : Nat := 1) : HostChip p where
  legal contribution := ∃ r : OutputRead p, contribution = busStateOf (r.interactions memBusId)
  singleton := True
...
/-- Recover an input-chip instance's stream chunk from its contribution: the `bytes` of *some*
    ...
    contribution isn't a legal read at all. This is what `Vm.inputChunk` should be, for an
    `openVm` built with the same `ptrReg`/`countReg`/`memBusId`. -/
...
/-- Recover the output chip's array from its contribution: the `words` of *some* witnessing
    ...
    `Vm.outputArray` should be, for an `openVm` built with the same `memBusId`. -/
noncomputable def outputArrayOf (memBusId : Nat) (contribution : BusState p) : Output p :=
  if h : ∃ r : OutputRead p, contribution = busStateOf (r.interactions memBusId)
  then h.choose.words else []

/-- A concrete OpenVM `Vm`: `defaultBusMap`'s four stateless lookup tables (default bus ids),
    memory initialization (`initialWord` gives each address's starting word) and finalization,
    the output chip, and an input chip that peeks registers `ptrReg`/`countReg` — all sharing
    `memBusId` for the (single) memory bus. -/
noncomputable def openVm (initialWord : ZMod p → ZMod p → Vector (ZMod p) 4)
    (ptrReg countReg : Nat) (memBusId : Nat := 1) : Vm p where
  hostChips :=
    [ pcLookupHostChip, bitwiseLookupHostChip, variableRangeCheckerHostChip,
      tupleRangeCheckerHostChip, memoryInitHostChip initialWord memBusId,
      memoryFinalizeHostChip memBusId, outputHostChip memBusId,
      inputHostChip ptrReg countReg memBusId ]
  inputChipType := ⟨7, by simp⟩
  inputChunk := inputChunkOf ptrReg countReg memBusId
  outputChipType := ⟨6, by simp⟩
  outputArray := outputArrayOf memBusId
