...
/-- Dropping an equal-payload `setNew`/`getPrevious` pair preserves the order-free discipline:
    at the pair's (shared, by payload equality) address, `S` and `R` add the same payload count
    to both fibers — whatever `setNewMult` is, including `setNewMult = -setNewMult` — so every
    per-address excess is unchanged. -/
  ...
  have key :
      (recvsAt shape addr M).map BusInteraction.payload
        ...
        = (recvsAt shape addr (S ::ₘ R ::ₘ M)).map BusInteraction.payload
          - (sendsAt shape addr (S ::ₘ R ::ₘ M)).map BusInteraction.payload := by
