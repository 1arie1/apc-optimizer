...
`docs/build.sh --pdf` builds a PDF (`docs/_out/tex/main.pdf`) from Verso's TeX output instead of
the site; it needs `xelatex` and `latexmk` (plus `rsvg-convert` for the figures), and stops after
writing `main.tex` if they are missing.
