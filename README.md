...
📖 A rendered, cross-linked walkthrough of the audited surface is published at **<https://powdr-labs.github.io/apc-optimizer/>** (see [`docs/`](./docs)), also available as a [PDF](https://powdr-labs.github.io/apc-optimizer/apc_optimizer.pdf).
...
- **Memory and execution-bridge interactions obey the order-free discipline and timestamp bound.** The `admissible` predicate is invariant under reordering the interaction list (see [`ApcOptimizer/MemoryBus.lean`](./ApcOptimizer/MemoryBus.lean)): per evaluated address it asserts only multiset counting (`admissibleMemoryBusM` — every matched receive consumes a send's record, at most one record enters a block from outside), plus TS_BOUND (`tsBounded`) — every message's declared timestamp slot (memory bus slot 6, execution bridge slot 1) carries a value below `2^29`, which is OpenVM's global timestamp argument. If either failed on real traces, completeness might be violated.
- **The block is entered at the pc the caller declares (ENTRY_KEY).** Only assumed when the caller supplies the block's entry pc (`entry_pc` / `block.blocks[0].start_pc` in the export, passed to `openVmOptimizer` as `entryPc`); with `none` the assumption is vacuous and the optimizer forgoes the optimizations that rest on it. Where supplied, `entryKeyed` ([`ApcOptimizer/MemoryBus.lean`](./ApcOptimizer/MemoryBus.lean)) asserts that the execution-bridge record entering the block from outside carries that pc — i.e. the row proves an execution of the block starting at its *first* instruction. Window atomicity alone does not give this: a *rotated* filling of the row (entered at an interior instruction, wrapping through the block's exit back to its start) yields the same message multiset with a different entry record, so it takes an assumption to exclude. Verify that the declared pc is the one the circuit's own constraints pin the first instruction's `from_state__pc` to, and — for a super-block of several basic blocks — that the first block is the entry. Being an `admissible` clause, a wrong pc can only cost completeness (a real trace the optimized circuit rejects), never soundness.
