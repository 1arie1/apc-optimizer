...
def parseOpenVm (contents : String) :
    Except String (Circuit babyBear × (Nat → Option OpenVmBusType) × Option Nat) :=
...
def parseSp1 (contents : String) :
    Except String (Circuit ApcOptimizer.SP1.koalaBear ×
      (Nat → Option ApcOptimizer.SP1.Sp1BusType) × Option Nat) :=
...
structure VmBackend (p : ℕ) (τ : Type) where
  parse : String → Except String (Circuit p × (Nat → Option τ) × Option Nat)
  optimize : (Nat → Option τ) → Option Nat → Circuit p → Circuit p × Derivations p
...
/-- Step the pipeline's three pass lists with per-pass timing, reporting the cumulative time spent in
    ...
    the exact lists `pipeline` runs (`ApcOptimizer/Implementation/Optimizer.lean`); per-pass timing
